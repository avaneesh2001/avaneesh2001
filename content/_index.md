---
date: 2024-06-24T04:14:54-08:00
draft: false
params:
  author: R Avaneesh
title: Home
weight: 10
---

{{< typeit 
  tag=h1
  lifeLike=true >}}
Hello! Welcome to my website. 🙏
{{< /typeit >}}

<div style="text-align: justify;">
Software Engineer with experience in building automation systems and AI‑driven solutions.  
My work spans telecom software development aligned with Open RAN specifications, large language model pipelines for code applications like unit test generation, automated code review, documentation generation inter alia, and practical automation tools that increase efficiency and reliability.
</div>

## Specializations

<div style="text-align: justify;">
• Automation Engineering  <br>
• Large Language Models (LLMs) & Prompt Engineering  <br>
• Telecom Software & O‑RAN Integration  <br>
• API Design & Test Framework Developmen  <br>
</div>

## Experience Summary
<div style="text-align: justify;">
• Software Engineer at Truminds Software Systems - Delivered automation for M‑Plane test cases within O‑RAN, integrated REST APIs, and enhanced telecom test frameworks. <br> 
• Built LLM‑based pipelines for automated unit test generation.  <br>
• Worked across Ubuntu and Windows environments using C#, Python, and Open RAN tooling.  <br>
</div>

<br>


{{< button href="experience" target="_self" >}}
See Complete Experience
{{< /button >}}

## Core Skills

<div style="text-align: justify;">
• Programming: Python, C#, C, C++  <br>
• AI/ML: LLMs, NLP, Prompt Engineering  <br>
• Tools & Systems: Linux (Ubuntu/Rocky/CentOS), REST APIs, Git
</div>

<br>


{{< button href="skills" target="_self" >}}
See All Skills
{{< /button >}}


## Education
<div style="text-align: justify;">
B.Tech in Electronics and Communication Engineering  
National Institute of Technology, Calicut  
Gold Medalist — Top Performer
</div>

<br>

{{< button href="education" target="_self" >}}
See Complete Educations
{{< /button >}}

A brief selection of academic and applied engineering projects. 👇
